...
A portable, project-local multi-agent system for turning **source materials
...
The bundle is just markdown skills/agents/prompts plus a few self-locating Python helpers,
so it isn't tied to one tool: copy it into `.claude/` (Claude Code), `.codex/`, `.cursor/`,
or any agent folder your harness reads from. Paths inside the bundle refer to the install
dir as `$BUNDLE` — substitute whichever folder you used.
...
```bash
# install into whatever agent folder your harness reads (.claude / .codex / .cursor / …)
./install.sh ~/my-notes            # default: ~/my-notes/.claude/{skills,agents,prompts,memory}
./install.sh ~/my-notes .codex     # or .cursor, etc.
#  (equivalently:  cp -R skills agents prompts memory  ~/my-notes/.claude/ )

cd ~/my-notes
cp ~/lecture*.pdf ~/past-exam*.pdf sources/         # source mode (topic mode needs no files)
# then ask your agent:  "把 sources/ 里的材料整理成一份复习讲义"   (or give a topic)
```
...
## Architecture (repo root = the portable bundle)
```
skills/
...
install.sh             # copy the four dirs into <project>/<harness-dir>/

# installed, the bundle lives under your harness folder, referred to as $BUNDLE:
<project>/.claude/{skills,agents,prompts,memory}     # or .codex/ , .cursor/ , …
```

Subagents are dispatched by name (`outline-architect`, `chapter-writer`, `chapter-auditor`)
in harnesses that auto-discover an `agents/` folder; elsewhere the same files are reusable as
role prompts. The Python helpers self-locate their resources, so they run from any install dir.
